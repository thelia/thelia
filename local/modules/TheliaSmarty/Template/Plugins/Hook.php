...
class Hook extends AbstractSmartyPlugin
{
    ...
    public function __construct($debug, ContainerAwareEventDispatcher $dispatcher, ParserContext $parserContext)
    {
        $this->debug       = $debug;
     ...
     */
    protected function autoInjectForm(array &$params)
    {
        if (null === $instance = $this->getParam($params, 'form')) {
            try {
                $params['form'] = $this->parserContext->getCurrentForm();
            } catch(SmartyPluginException $e) {
                // if there are no current form
            }
        }
    }
     ...
     */
    public function processHookFunction(array $params, &$smarty)
    {
     ...
     */
    protected function moduleIncludeCompat(array $params, &$smarty)
    {
    ...
    protected function showHook($hookName, array $params)
    {
     ...
     */
    public function processHookBlock(array $params, $content, $smarty, &$repeat)
    {
        $hookName = $this->getParam($params, 'name');
        $module   = intval($this->getParam($params, 'module', 0));
     ...
     * @return string                    the generated content
     */
    public function processForHookBlock(array $params, $content, $smarty, &$repeat)
    {
     ...
     * @return string the hook output
     */
    public function ifHook(array $params, $content, /** @noinspection PhpUnusedParameterInspection */ $template, &$repeat)
    {
        // When encountering close tag, check if hook has results.
        if ($repeat === false) {
            return $this->checkEmptyHook($params) ? '' : $content;
     ...
     * @throws \InvalidArgumentException
     */
    protected function checkEmptyHook(array $params)
    {
     ...
     * @return array
     */
    protected function getArgumentsFromParams(array $params)
    {
        $args     = array();
