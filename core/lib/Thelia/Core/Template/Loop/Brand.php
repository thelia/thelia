...
class Brand extends BaseI18nLoop implements PropelSearchLoopInterface, SearchLoopInterface
{
    ...
    public function buildModelCriteria(): ModelCriteria
    {
        ...
        if (true === $current) {
            $search->filterById(($this->getMainRequest()->attributes->get('brand_id') ?? $this->getMainRequest()->query->get('brand_id')));
        } elseif (false === $current) {
            $search->filterById(($this->getMainRequest()->attributes->get('brand_id') ?? $this->getMainRequest()->query->get('brand_id')), Criteria::NOT_IN);
        }
