...
class Content extends BaseI18nLoop implements PropelSearchLoopInterface, SearchLoopInterface
{
    ...
    public function buildModelCriteria(): ModelCriteria
    {
        ...
        if (true === $current) {
            $search->filterById(($this->getMainRequest()->attributes->get('content_id') ?? $this->getMainRequest()->query->get('content_id')));
        } elseif (false === $current) {
            $search->filterById(($this->getMainRequest()->attributes->get('content_id') ?? $this->getMainRequest()->query->get('content_id')), Criteria::NOT_IN);
        }

        $current_folder = $this->getCurrentFolder();

        if (true === $current_folder) {
            $current = ContentQuery::create()->findPk(($this->getMainRequest()->attributes->get('content_id') ?? $this->getMainRequest()->query->get('content_id')));
