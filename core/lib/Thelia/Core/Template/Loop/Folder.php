...
class Folder extends BaseI18nLoop implements PropelSearchLoopInterface, SearchLoopInterface
{
    ...
    public function buildModelCriteria(): ModelCriteria
    {
        ...
        if (true === $current) {
            $search->filterById(($this->getMainRequest()->attributes->get('folder_id') ?? $this->getMainRequest()->query->get('folder_id')));
        } elseif (false === $current) {
            $search->filterById(($this->getMainRequest()->attributes->get('folder_id') ?? $this->getMainRequest()->query->get('folder_id')), Criteria::NOT_IN);
        }
