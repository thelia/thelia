<?php

declare(strict_types=1);

/*
 * This file is part of the Thelia package.
 * http://www.thelia.net
 *
 * (c) OpenStudio <info@thelia.net>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace Thelia\Command\Import\Importer;

use Thelia\Command\Import\AbstractDemoImporter;
use Thelia\Command\Import\DemoImportContext;
use Thelia\Model\Feature;
use Thelia\Model\FeatureAv;

final class MaterialsImporter extends AbstractDemoImporter
{
    public function priority(): int
    {
        return 10;
    }

    public function description(): string
    {
        return 'Materials feature';
    }

    public function import(DemoImportContext $context): void
    {
        $feature = (new Feature())
            ->setPosition(1)
            ->setLocale('fr_FR')->setTitle('Matière')
            ->setLocale('en_US')->setTitle('Material');

        $position = 0;
        foreach ($this->readCsv($context->dataDir.'materials.csv', skipHeader: false) as $data) {
            $feature->addFeatureAv(
                (new FeatureAv())
                    ->setPosition(++$position)
                    ->setLocale('fr_FR')->setTitle($data[0])
                    ->setLocale('en_US')->setTitle($data[1]),
            );
        }

        $feature->save($context->connection);
        $context->setMaterialsFeature($feature);
    }
}
