...
abstract class AdminProfileQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @param array[$admin_id, $profile_id] $key Primary key to use for the query
    ...
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = AdminProfileTableMap::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT `ADMIN_ID`, `PROFILE_ID`, `CREATED_AT`, `UPDATED_AT` FROM `admin_profile` WHERE `ADMIN_ID` = :p0 AND `PROFILE_ID` = :p1';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            AdminProfileTableMap::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(AdminProfileTableMap::ADMIN_ID, $key[0], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(AdminProfileTableMap::ADMIN_ID, $key[0], Criteria::EQUAL);
    ...
    {
        if ($adminProfile) {
            $this->addCond('pruneCond0', $this->getAliasedColName(AdminProfileTableMap::ADMIN_ID), $adminProfile->getAdminId(), Criteria::NOT_EQUAL);
