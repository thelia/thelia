...
class RewritingUrl extends BaseRewritingUrl
{
    ...
    {
        // Unlike 2.6, T3's own URL generation (UrlRewritingTrait::generateRewrittenUrl())
        // builds and resolves ".html"-suffixed candidates to detect slug collisions, so the
        // default here keeps the extension untouched. Set the config key to enable stripping.
        if (ConfigQuery::read(self::REMOVE_HTML_CONFIG_KEY, false)) {
        ...
        return strtolower(\is_string($sanitizedUrl) ? $sanitizedUrl : $url);
    }

    protected function unifyUrl(string $cleanedUrl): string
    {
        ...
        return $this->unifyUrl($this->getViewId().'-'.$cleanedUrl);
    }

    protected function replaceSpace(string $string, string $replacement = '-'): string
    {
        return preg_replace('/\s+/', $replacement, $string) ?? $string;
    }

    protected function replaceMultipleHyphens(string $string, string $replacement = '-'): string
    {
    ...
    protected function removeHtmlExtension(string $string, string $replacement = ''): string
    {
        return str_ireplace('.html', $replacement, $string);
    }

    protected function convertAccents(string $string): string
    {
