<?php

declare(strict_types=1);

/*
 * This file is part of the Thelia package.
 * http://www.thelia.net
 *
 * (c) OpenStudio <info@thelia.net>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

return [
    'Update on your return {{ return_ref }}' => 'Aggiornamento sul tuo reso {{ return_ref }}',
    'A {{ config("store_name") }} account has been created for you' => 'È stato creato un account {{ config("store_name") }} per te',
    'Address' => 'Indirizzo',
    'Advanced configuration' => 'Configurazione avanzata',
    'Afghanistan' => 'Afghanistan',
    'Agrigento' => 'Agrigento',
    'Akita' => 'Akita',
    'Alabama' => 'Alabama',
    'Åland Islands' => 'Isole Åland',
    'Alaska' => 'Alaska',
    'Albania' => 'Albania',
    'Alberta' => 'Alberta',
    'Alessandria' => 'Alessandria',
    'Algeria' => 'Algeria',
    'American Samoa' => 'Samoa Americane',
    'Ancona' => 'Ancona',
    'Andorra' => 'Andorra',
    'Angola' => 'Angola',
    'Anguilla' => 'Anguilla',
    'Antarctica' => 'Antartide',
    'Antigua and Barbuda' => 'Antigua e Barbuda',
    'Aosta' => 'Aosta',
    'API Configuration' => 'Configurazione API',
    'Arezzo' => 'Arezzo',
    'Argentina' => 'Argentina',
    'Arizona' => 'Arizona',
    'Arkansas' => 'Arkansas',
    'Armenia' => 'Armenia',
    'Aruba' => 'Aruba',
    'Ascoli Piceno' => 'Ascoli Piceno',
    'Asti' => 'Asti',
    'Australia' => 'Australia',
    'Austria' => 'Austria',
    'Avellino' => 'Avellino',
    'Azerbaijan' => 'Azerbaigian',
    'Bahamas' => 'Bahamas',
    'Bahrain' => 'Bahrein',
    'Bali' => 'Bali',
    'Bangladesh' => 'Bangladesh',
    'Barbados' => 'Barbados',
    'Bari' => 'Bari',
    'Belarus' => 'Bielorussia',
    'Belgium' => 'Belgio',
    'Belize' => 'Belize',
    'Belluno' => 'Belluno',
    'Benevento' => 'Benevento',
    'Benin' => 'Benin',
    'Bergamo' => 'Bergamo',
    'Bermuda' => 'Bermuda',
    'Bhutan' => 'Bhutan',
    'Biella' => 'Biella',
    'Bielorussia' => 'Bielorussia',
    'Bolivia' => 'Bolivia',
    'Bologna' => 'Bologna',
    'Bonaire, Sint Eustatius and Saba' => 'Bonaire, Sint Eustatius e Saba',
    'Bosnia and Herzegovina' => 'Bosnia ed Erzegovina',
    'Botswana' => 'Botswana',
    'Bouvet Island' => 'Isola Bouvet',
    'Brazil' => 'Brasile',
    'Brescia' => 'Brescia',
    'Brindisi' => 'Brindisi',
    'British Indian Ocean Territory' => 'Territorio britannico dell\'Oceano Indiano',
    'Brunei' => 'Brunei',
    'Buenos Aires' => 'Buenos Aires',
    'Bulgaria' => 'Bulgaria',
    'Burkina' => 'Burkina Faso',
    'Burma' => 'Birmania',
    'Burundi' => 'Burundi',
    'Cagliari' => 'Cagliari',
    'California' => 'California',
    'Caltanissetta' => 'Caltanissetta',
    'Cambodia' => 'Cambogia',
    'Cameroon' => 'Camerun',
    'Campobasso' => 'Campobasso',
    'Canada' => 'Canada',
    'Cape Verde' => 'Capo Verde',
    'Caserta' => 'Caserta',
    'Catania' => 'Catania',
    'Catanzaro' => 'Catanzaro',
    'Category' => 'Categoria',
    'Cayman Islands' => 'Isole Cayman',
    'Central African Republic' => 'Rep. Centrafricana',
    'Chad' => 'Ciad',
    'Chieti' => 'Chieti',
    'Chile' => 'Cile',
    'China' => 'Cina',
    'Christmas Island' => 'Isola Christmas',
    'Cocos (Keeling) Islands' => 'Isole Cocos (Keeling)',
    'Colombia' => 'Colombia',
    'Colorado' => 'Colorado',
    'Como' => 'Como',
    'Comoros' => 'Comore',
    'Configuration' => 'Impostazioni',
    'Confirm your %store account' => 'Conferma il tuo account {{ config("store_name") }}',
    'Congo' => 'Rep. del Congo',
    'Content' => 'Contenuto',
    'Cook Islands' => 'Isole Cook',
    'Cosenza' => 'Cosenza',
    'Costa Rica' => 'Costa Rica',
    'Coupon' => 'Promozione',
    'Cremona' => 'Cremona',
    'Croatia' => 'Croazia',
    'Crotone' => 'Crotone',
    'Cuba' => 'Cuba',
    'Cuneo' => 'Cuneo',
    'Curaçao' => 'Curaçao',
    'Customer' => 'Cliente',
    'Cyprus' => 'Cipro',
    'Czech Republic' => 'Rep. Ceca',
    'Denmark' => 'Danimarca',
    'Djibouti' => 'Gibuti',
    'Dominica' => 'Dominica',
    'Dominican Republic' => 'Rep. Dominicana',
    'Ecuador' => 'Ecuador',
    'Egypt' => 'Egitto',
    'El Salvador' => 'El Salvador',
    'Equatorial Guinea' => 'Guinea Equatoriale',
    'Eritrea' => 'Eritrea',
    'Estonia' => 'Estonia',
    'Ethiopia' => 'Etiopia',
    'Falkland Islands (Malvinas)' => 'Isole Falkland (Malvine)',
    'Faroe Islands' => 'Isole Fær Øer',
    'Ferrara' => 'Ferrara',
    'Fiji' => 'Figi',
    'Finland' => 'Finlandia',
    'Firenze' => 'Firenze',
    'Florida' => 'Florida',
    'Foggia' => 'Foggia',
    'Folder' => 'Cartella',
    'Forlì-Cesena' => 'Forlì-Cesena',
    'France metropolitan' => 'Francia',
    'French Southern Territories' => 'Terre australi e antartiche francesi',
    'Gabon' => 'Gabon',
    'Gambia' => 'Gambia',
    'Genova' => 'Genova',
    'Georgia' => 'Georgia',
    'Germany' => 'Germania',
    'Ghana' => 'Ghana',
    'Gibraltar' => 'Gibilterra',
    'Gorizia' => 'Gorizia',
    'Greece' => 'Grecia',
    'Greenland' => 'Groenlandia',
    'Grenada' => 'Grenada',
    'Guadeloupe' => 'Guadalupa',
    'Guam' => 'Guam',
    'Guatemala' => 'Guatemala',
    'Guernsey' => 'Guernsey',
    'Guinea' => 'Guinea',
    'Guinea-Bissau' => 'Guinea-Bissau',
    'Guyana' => 'Guyana',
    'Guyane Française' => 'Guyana francese',
    'Haiti' => 'Haiti',
    'Heard Island and McDonald Islands' => 'Isole Heard e McDonald',
    'Honduras' => 'Honduras',
    'Hong Kong' => 'Hong Kong',
    'Hook positions' => 'Posizioni dell\'hook',
    'Hungary' => 'Ungheria',
    'Iceland' => 'Islanda',
    'Idaho' => 'Idaho',
    'Illinois' => 'Illinois',
    'Imperia' => 'Imperia',
    'India' => 'India',
    'Indonesia' => 'Indonesia',
    'Iran' => 'Iran',
    'Iraq' => 'Iraq',
    'Ireland' => 'Irlanda',
    'Isle of Man' => 'Isola di Man',
    'Israel' => 'Israele',
    'Italy' => 'Italia',
    'Ivory Coast' => 'Costa d\'Avorio',
    'Jakarta' => 'Jakarta',
    'Jamaica' => 'Giamaica',
    'Japan' => 'Giappone',
    'Jersey' => 'Jersey',
    'Jordan' => 'Giordania',
    'Kansas' => 'Kansas',
    'Kazakhstan' => 'Kazakistan',
    'Kentucky' => 'Kentucky',
    'Kenya' => 'Kenya',
    'Kiribati' => 'Kiribati',
    'Kuwait' => 'Kuwait',
    'Kyrgyzstan' => 'Kirghizistan',
    'L\'Aquila' => 'L\'Aquila',
    'La Spezia' => 'La Spezia',
    'language selection page - at the top' => 'pagina selezione lingua - nella parte superiore',
    'Laos' => 'Laos',
    'Lari' => 'Lari',
    'Latina' => 'Latina',
    'Latvia' => 'Lettonia',
    'Lebanon' => 'Libano',
    'Lecce' => 'Lecce',
    'Lecco' => 'Lecco',
    'Lesotho' => 'Lesotho',
    'Liberia' => 'Liberia',
    'Libya' => 'Libia',
    'Liechtenstein' => 'Liechtenstein',
    'Lithuania' => 'Lituania',
    'Livorno' => 'Livorno',
    'Lodi' => 'Lodi',
    'Louisiana' => 'Louisiana',
    'Lucca' => 'Lucca',
    'Luxembourg' => 'Lussemburgo',
    'Macao' => 'Macao',
    'Macedonia' => 'Macedonia del Nord',
    'Macerata' => 'Macerata',
    'Madagascar' => 'Madagascar',
    'Maine' => 'Maine',
    'Malawi' => 'Malawi',
    'Malaysia' => 'Malaysia',
    'Maldives' => 'Maldive',
    'Mali' => 'Mali',
    'Malta' => 'Malta',
    'Mantova' => 'Mantova',
    'Marshall Islands' => 'Isole Marshall',
    'Martinique' => 'Martinica',
    'Maryland' => 'Maryland',
    'Massa' => 'Massa',
    'Massachusetts' => 'Massachusetts',
    'Matera' => 'Matera',
    'Mauritania' => 'Mauritania',
    'Mauritius' => 'Mauritius',
    'Mayotte' => 'Mayotte',
    'Medio Campidano' => 'Medio Campidano',
    'Messina' => 'Messina',
    'Mexico' => 'Messico',
    'Michigan' => 'Michigan',
    'Micronesia' => 'Micronesia',
    'Milano' => 'Milano',
    'Minnesota' => 'Minnesota',
    'Mississippi' => 'Mississippi',
    'Missouri' => 'Missouri',
    'Modena' => 'Modena',
    'Moldova' => 'Moldavia',
    'Monaco' => 'Monaco',
    'Mongolia' => 'Mongolia',
    'Montana' => 'Montana',
    'Montenegro' => 'Montenegro',
    'Montserrat' => 'Montserrat',
    'Morocco' => 'Marocco',
    'Mozambique' => 'Mozambico',
    'Namibia' => 'Namibia',
    'Napoli' => 'Napoli',
    'Nauru' => 'Nauru',
    'Nepal' => 'Nepal',
    'Netherlands' => 'Paesi Bassi',
    'Nevada' => 'Nevada',
    'New Hampshire' => 'New Hampshire',
    'New Jersey' => 'New Jersey',
    'New order {{ order_ref }} placed on {{ config("store_name") }}' => 'Nuovo ordine {{ order_ref }} su {{ config("store_name") }}',
    'New password request on %store' => 'Richiesta di una nuova password su {{ config("store_name") }}',
    'New York' => 'Roma',
    'New Zealand' => 'Nuova Zelanda',
    'Nicaragua' => 'Nicaragua',
    'Niger' => 'Niger',
    'Nigeria' => 'Nigeria',
    'Niue' => 'Niue',
    'Norfolk Island' => 'Isola Norfolk',
    'North Carolina' => 'North Carolina',
    'North Dakota' => 'North Dakota',
    'North Korea' => 'Corea del Nord',
    'Northern Mariana Islands' => 'Isole Marianne Settentrionali',
    'Norway' => 'Norvegia',
    'Nouvelle-Calédonie' => 'Nuova Caledonia',
    'Novara' => 'Novara',
    'Ohio' => 'Ohio',
    'Oklahoma' => 'Oklahoma',
    'Olbia-Tempio' => 'Olbia-Tempio',
    'Oman' => 'Oman',
    'Ontario' => 'Ontario',
    'Oregon' => 'Oregon',
    'Oristano' => 'Oristano',
    'Padova' => 'Padova',
    'Pakistan' => 'Pakistan',
    'Palau' => 'Palau',
    'Palermo' => 'Palermo',
    'Panama' => 'Panama',
    'Papua Nueva Guinea' => 'Papua Nuova Guinea',
    'Paraguay' => 'Paraguay',
    'Parma' => 'Parma',
    'Pavia' => 'Pavia',
    'Pennsylvania' => 'Pennsylvania',
    'Peru' => 'Perù',
    'Perugia' => 'Perugia',
    'Pesaro-Urbino' => 'Pesaro-Urbino',
    'Pescara' => 'Pescara',
    'Philippines' => 'Filippine',
    'Piacenza' => 'Piacenza',
    'Pisa' => 'Pisa',
    'Pistoia' => 'Pistoia',
    'Pitcairn' => 'Pitcairn',
    'Poland' => 'Polonia',
    'Polynésie française' => 'Polinesia francese',
    'Pordenone' => 'Pordenone',
    'Portugal' => 'Portogallo',
    'Potenza' => 'Potenza',
    'Prato' => 'Prato',
    'Product' => 'Prodotto',
    'Puerto Rico' => 'Puerto Rico',
    'Qatar' => 'Qatar',
    'Quebec' => 'Quebec',
    'Ragusa' => 'Ragusa',
    'Ravenna' => 'Ravenna',
    'Reggio Calabria' => 'Reggio Calabria',
    'Reggio Emilia' => 'Reggio Emilia',
    'Reset your password on {{ config("store_name") }}' => 'Reimposta la tua password su {{ config("store_name") }}',
    'Rieti' => 'Rieti',
    'Rimini' => 'Rimini',
    'Romania' => 'Romania',
    'Russia' => 'Russia',
    'Rwanda' => 'Ruanda',
    'Réunion(La)' => 'La Riunione',
    'Saint Barthélemy' => 'Saint-Barthélemy',
    'Saint Helena, Ascension and Tristan da Cunha' => 'Sant\'Elena, Ascensione e Tristan da Cunha',
    'Saint Kitts and Nevis' => 'Saint Kitts e Nevis',
    'Saint Lucia' => 'Saint Lucia',
    'Saint Martin (French part)' => 'Saint-Martin',
    'Saint Vincent and the Grenadines' => 'Saint Vincent e Grenadine',
    'Salerno' => 'Salerno',
    'Sales management' => 'Gestione vendite',
    'San Marino' => 'San Marino',
    'Santa Cruz' => 'Santa Cruz',
    'Sao Tome and Principe' => 'São Tomé e Príncipe',
    'Sassari' => 'Sassari',
    'Saudi Arabia' => 'Arabia Saudita',
    'Savona' => 'Savona',
    'Search' => 'Ricerca',
    'Senegal' => 'Senegal',
    'Serbia' => 'Serbia',
    'Seychelles' => 'Seychelles',
    'Shipping configuration' => 'Configurazione di spedizione',
    'Siena' => 'Siena',
    'Sierra Leone' => 'Sierra Leone',
    'Singapore' => 'Singapore',
    'Sint Maarten (Dutch part)' => 'Sint Maarten (parte olandese)',
    'Siracusa' => 'Siracusa',
    'Slovakia' => 'Slovacchia',
    'Slovenia' => 'Slovenia',
    'Solomon Islands' => 'Isole Salomone',
    'Somalia' => 'Somalia',
    'Sondrio' => 'Sondrio',
    'South Africa' => 'Sudafrica',
    'South Georgia and the South Sandwich Islands' => 'Georgia del Sud e Isole Sandwich Australi',
    'South Korea' => 'Corea del Sud',
    'South Sudan' => 'Sud Sudan',
    'Spain' => 'Spagna',
    'Sri Lanka' => 'Sri Lanka',
    'St Pierre et Miquelon' => 'Saint-Pierre e Miquelon',
    'State of Palestine' => 'Stato di Palestina',
    'Sud Sardegna' => 'Sud Sardegna',
    'Sudan' => 'Sudan',
    'Suriname' => 'Suriname',
    'Svalbard and Jan Mayen' => 'Svalbard e Jan Mayen',
    'Swaziland' => 'eSwatini',
    'Sweden' => 'Svezia',
    'Switzerland' => 'Svizzera',
    'Syria' => 'Siria',
    'Taïwan' => 'Taiwan',
    'Tajikistan' => 'Tagikistan',
    'Tanzania' => 'Tanzania',
    'Taranto' => 'Taranto',
    'tax rule - Edit JavaScript' => 'regola fiscale - Modificare JavaScript',
    'Teramo' => 'Teramo',
    'Terni' => 'Terni',
    'Thailand' => 'Thailandia',
    'The Vatican' => 'Città del Vaticano',
    'Timor-Leste' => 'Timor Est',
    'tinymce wysiwyg editor' => 'tinymce wysiwyg editor',
    'Togo' => 'Togo',
    'Tokelau' => 'Tokelau',
    'Tokyo' => 'Tokyo',
    'Tonga' => 'Tonga',
    'Torino' => 'Torino',
    'Trapani' => 'Trapani',
    'Trento' => 'Trento',
    'Treviso' => 'Treviso',
    'Trieste' => 'Trieste',
    'Trinidad and Tobago' => 'Trinidad e Tobago',
    'Tunisia' => 'Tunisia',
    'Turkey' => 'Turchia',
    'Turkmenistan' => 'Turkmenistan',
    'Turks and Caicos Islands' => 'Isole Turks e Caicos',
    'Tuvalu' => 'Tuvalu',
    'Udine' => 'Udine',
    'Uganda' => 'Uganda',
    'Ukraine' => 'Ucraina',
    'United Arab Emirates' => 'Emirati Arabi Uniti',
    'United Kingdom' => 'Regno Unito',
    'United States Minor Outlying Islands' => 'Isole minori esterne degli Stati Uniti',
    'Uruguay' => 'Uruguay',
    'USA' => 'Stati Uniti',
    'Uzbekistan' => 'Uzbekistan',
    'Vanuatu' => 'Vanuatu',
    'Varese' => 'Varese',
    'Venezia' => 'Venezia',
    'Venezuela' => 'Venezuela',
    'Vercelli' => 'Vercelli',
    'Vermont' => 'Vermont',
    'Verona' => 'Verona',
    'Vibo Valentia' => 'Vibo Valentia',
    'Vicenza' => 'Vicenza',
    'Vietnam' => 'Vietnam',
    'Virgin Islands (British)' => 'Isole Vergini Britanniche',
    'Virgin Islands (U.S.)' => 'Isole Vergini Americane',
    'Viterbo' => 'Viterbo',
    'Wallis-et-Futuna' => 'Wallis e Futuna',
    'Western Sahara' => 'Sahara Occidentale',
    'Western Samoa' => 'Samoa',
    'Yemen' => 'Yemen',
    'Your %store activation code' => 'Il tuo codice di attivazione {{ config("store_name") }}',
    'Your account information on {{ config("store_name") }} has been changed.' => 'Le informazioni del tuo account su {{ config("store_name") }} sono state modificate.',
    'Your order {{ order_ref }} at {{ config("store_name") }}' => 'Il tuo ordine {{ order_ref }} su {{ config("store_name") }}',
    'Your subscription to %store newsletter' => 'La tua iscrizione alla newsletter di {{ config("store_name") }}',
    'Yucatán' => 'Yucatán',
    'Zaire' => 'RD del Congo',
    'Zambia' => 'Zambia',
    'Zimbabwe' => 'Zimbabwe',
];
