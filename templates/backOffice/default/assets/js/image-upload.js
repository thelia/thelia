$(function ($) {
  ...
  $.imageUploadManager.updateImageListAjax = function () {
    ...
    $.ajax({
      ...
      statusCode: {
        404: function () {
          $imageListArea.html(imageListErrorMessage);
        },
      },
    }).done(function (data) {
      ...
      if ($count == 0) {
        ...
        $btnCancel.html(
          '<span class="glyphicon glyphicon-remove"></span>' +
            textBtnDeleteImageModal_No
        );
        if ($count == 1) {
          $warning = imageDeleteOneWarningMessage;
        } else {
          $warning = imageDeleteMultipleWarningMessage.replace(
            "%count",
            $count
          );
        }
  ...
  $.imageUploadManager.deleteImage = function ($id) {
    ...
    $.ajax({
      ...
      statusCode: {
        404: function () {
          $(".image-manager .message").html(errorMessage);
        },
      },
    })
      .done(function (data) {
        $greatParent.parent().remove();
        $(".image-manager .message").html(data);
    ...
    $("#js-sort-image").sortable({
      ...
      stop: function (event, ui) {
        ...
        $.ajax({
          ...
          statusCode: {
            404: function () {
              $(".image-manager .message").html(imageReorderErrorMessage);
            },
          },
        }).done(function (data) {
          $(".image-manager .message").html(data);
