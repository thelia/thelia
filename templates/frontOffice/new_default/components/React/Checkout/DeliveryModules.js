...
import useValidDeliveryModules from '../Checkout/hooks/useValidDeliveryModules';
import {
  queryClient,
  useGetCheckout,
  useSetCheckout
  ...
  return (
    <label htmlFor={`option_${option?.code}`} className="Radio">
      ...
      <div className='flex flex-wrap'>
        <span className={`block mr-6 text-base ${isSelected ? 'text-main' : ''}`}>
          {option.title || module?.i18n?.title}
        </span>
        <strong className="text-main">
          {option.postage ? `${priceFormat(option.postage)}` : intl.formatMessage({ id: 'FREE' })}
        </strong>
      </div>
    </label >
  );
      ...
      {isLoading ? (
        ...
        (modules?.length === 0 ||
          ...
          <div className="flex flex-col gap-3 mt-8 flex-start item-start">
            <Title className="mb-5 text-2xl Title--3" title='CHOOSE_DELIVERY_PROVIDER' />
            {modules.map((module) =>
